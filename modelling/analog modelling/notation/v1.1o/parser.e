...
## Looks for a remove (allow forget too) statement (allows forgeting of definitions)

ANparse_remove = [ "prefix", "remove ", "ANparse_name", [ "fail", "ANparse_forget"],
			[ "action", ["later","ANparse_doRemove($p1[1]);"]]];
ANparse_forget = [ "prefix", "forget ", "ANparse_name", [ "fail", "ANparse_depend"],
			[ "action", ["later","ANparse_doRemove($p1[1]);"]]];
...
proc ANparse_doRemove {
	ANparse_doRemove_tmp=forget($1);
	if (ANparse_doRemove_tmp != 0) 
	{
		error($1[1] // " can not be removed.\n\n");
	}
	else
	{
		todo("AN_removeVar(\"" // $1[1] // "\");");
	}
}
