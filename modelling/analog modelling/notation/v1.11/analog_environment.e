...
string acceptButton_txtCol = "black";
...
safe_to_execute is (AN_mode_potset==1 || AN_mode_hold==1 || AN_mode_pc==1);

acceptButton_txtCol is safe_to_execute ? "black" : "DarkSlateGray4";

proc executeCode : ~acceptButton_mouse_1 {
	if (~acceptButton_mouse_1[2] == 4  && safe_to_execute) {
	...
	else if (~acceptButton_mouse_1[2] == 5  && safe_to_execute) {
...
proc ANdisp_doScrollUp : ANdisp_scrollup {
	if (ANdisp_scrollup==1) {
		ANdisp_dummyWindow_y=ANdisp_dummyWindow_y+8;
...
## Unccoment to make look a bit nicer on linux
## input_x2=49;